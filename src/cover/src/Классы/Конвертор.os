...
Процедура ПодготовитьЛог(ИсходныйТекст) Экспорт
	...
	ПолеДанныеПокрытия.РазобратьЛог(Лог, ДанныеКонфигурации, ДанныеОбработок);
...
Процедура СохранитьРезультат(КаталогРезультатов) Экспорт
	...
	СохранитьВlcov(КаталогРезультатов);
...
Процедура СохранитьВlcov(КаталогРезультатов)
	...
	Для Каждого Модуль Из ПолеДанныеПокрытия.Результат Цикл
		...
		// FixME Это надо делать в формировании данных покрытия
	
		СтрокиМодуля = Новый Соответствие();
		Для Каждого СтрокаМодуля Из Модуль.Значение.Строки Цикл

			Попытка
				ДанныеСтрокиПокрытия = СтрокиМодуля[СтрокаМодуля.НомерСтроки];
			Исключение
				ДанныеСтрокиПокрытия = Новый Структура("НомерСтроки, Хинт", СтрокаМодуля.НомерСтроки, 0);
			КонецПопытки;
			
			Если ДанныеСтрокиПокрытия = Неопределено Тогда
				ДанныеСтрокиПокрытия = Новый Структура("НомерСтроки, Хинт", СтрокаМодуля.НомерСтроки, 0);
			КонецЕсли;

			ДанныеСтрокиПокрытия.Хинт = ДанныеСтрокиПокрытия.Хинт + 1;

			СтрокиМодуля.Вставить(ДанныеСтрокиПокрытия.НомерСтроки, ДанныеСтрокиПокрытия);

		КонецЦикла;	

		Для Каждого СтрокаМодуляСХинтами Из СтрокиМодуля Цикл

			СтрокиДляЗаписи.Добавить(
				"DA:" + Строка(СтрокаМодуляСХинтами.Значение.НомерСтроки) + "," + Строка(СтрокаМодуляСХинтами.Значение.Хинт)
				);
