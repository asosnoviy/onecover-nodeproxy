...
Процедура ПодготовитьЛог(ИсходныйТекст) Экспорт
	...
	// ПомошникЗаписи.ЗаписатьВJson(Лог, "./out/log.json");

	ПолеДанныеПокрытия.РазобратьЛог(Лог, ДанныеКонфигурации);

КонецПроцедуры	

Процедура УстановитьФайлКонфигурации(ПутьКФайлуКонфигурации) Экспорт
...
Процедура СохранитьВGenericCoverage(КаталогРезультатов)
	...
	ПутиКИсходникам = "C:\DEVELOP\sonar\bases\ssl-cover\src\";
	...
	Для Каждого Модуль Из ПолеДанныеПокрытия.Результат Цикл
		...
		ЗаписьXML.ЗаписатьАтрибут("path", ПутиКИсходникам + Модуль.Значение.Путь);
